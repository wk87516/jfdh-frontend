<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="100px">
      ...
      <el-form-item label="操作员" prop="operator" v-if="autoQuery && (queryParams.czydm || queryParams.czyxm)">
        <span>{{ queryParams.czyxm || queryParams.czydm || '未知' }}</span>
        <span v-if="queryParams.czydm && queryParams.czyxm" class="text-gray">（{{ queryParams.czydm }}）</span>
      </el-form-item>

      <el-form-item v-if="!autoQuery">
    ...
    <el-alert v-else-if="!loading && !autoQuery" title="暂无数据" type="warning" :closable="false" style="margin-bottom: 10px;">
      ...
      <el-row :gutter="20" class="statistics">
        ...
        <el-col :span="8">
          <div class="stat-item">
            ...
            <div class="stat-value">{{ formatPoints(totalPointsRequired) }}</div>
          </div>
        </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    loadUrlParams() {
      ...
      console.log('[独立页面] URL参数:', query);

      // 获取URL参数
      const ghlsh = query.ghlsh || '';
      const czydm = query.czydm || '';
      const czyxm = query.czyxm || '';

      if (ghlsh) {
        this.queryParams.ghlsh = ghlsh;
        this.queryParams.czydm = czydm;
        this.queryParams.czyxm = czyxm;
        this.autoQuery = true;

        console.log('[独立页面] 从URL获取参数 - 就诊流水号:', ghlsh, '操作员代码:', czydm, '操作员姓名:', czyxm);
    ...
    handleQuery() {
      ...
      queryPaymentItemsWithExchange(this.queryParams.ghlsh).then(response => {
        console.log('[独立页面] 完整响应:', response);
        console.log('[独立页面] 响应code:', response.code);
        console.log('[独立页面] 响应data:', response.data);
        console.log('[独立页面] items:', response.data && response.data.items);

        // 后端返回结构：{code: 200, msg: "操作成功", data: {summary: {...}, items: [...]}}
        this.paymentItems = (response.code === 200 && response.data && response.data.items) || [];
        this.selectedItems = [];

        console.log('[独立页面] 解析后的paymentItems:', this.paymentItems);
        console.log('[独立页面] paymentItems数量:', this.paymentItems.length);

        // 查询患者积分账户
        // 从HIS查询结果中的patientId
        const patientIdToQuery = this.paymentItems.length > 0 ? this.paymentItems[0].patientId : null;

        if (patientIdToQuery) {
          console.log('[独立页面] 查询患者积分账户，患者ID:', patientIdToQuery);
          this.loadPatientAccount(patientIdToQuery);
        } else {
          console.log('[独立页面] 未找到患者ID，无法查询积分账户');
          this.patientAccount = null;
        }
      }).catch(error => {
        console.error('[独立页面] 查询失败:', error);
    ...
    performExchange() {
      const exchangeRequest = {
        ...
        // 使用URL参数中的操作员信息，如果没有则使用默认值
        operatorId: this.queryParams.czydm || 'SYSTEM',
        operatorName: this.queryParams.czyxm || '系统'
      };

      console.log('[独立页面] 兑换请求，操作员:', exchangeRequest.operatorId, exchangeRequest.operatorName);
